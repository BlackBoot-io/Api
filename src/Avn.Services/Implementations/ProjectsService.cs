...
public class ProjectsService : IProjectsService
{
    ...
    public async Task<IActionResponse<IEnumerable<object>>> GetAllAsync(Guid userId, CancellationToken cancellationToken = default)
        => new ActionResponse<IEnumerable<object>>(await _uow.ProjectRepo
    ...
    {
        var model = await _uow.ProjectRepo.FirstOrDefaultAsync(x => x.Id == item.Id && x.UserId == item.UserId, cancellationToken);
        if (model is null)
    ...
    {
        var model = await _uow.ProjectRepo.FirstOrDefaultAsync(x => x.Id == projectId && x.UserId == userId, cancellationToken);
        if (model is null)
            return new ActionResponse<Guid>(ActionResponseStatusCode.NotFound, BusinessMessage.RecordNotFound);
